...
public class Day24 {
	...
	static long part2() {
		...
		for (int i = 0; i < 200; i++) {
			if (i % 2 == 0) {
				int min = levels.keySet()
						.stream()
						.min(Integer::compare)
						.get();
				int max = levels.keySet()
						.stream()
						.max(Integer::compare)
						.get();
...
//		}
	...
	private static void walk(Map<Integer, boolean[][]> levels, int i, Map<Integer, boolean[][]> ret,
		...
		for (Point2D cell : outerEdges) {
			...
			int outerNeighbors = (int) checkInUpper.stream()
					.filter(c -> outer[c.y][c.x])
					.count();
		...
		for (Point2D cell : innerEdges) {
			...
			int innerNeighbors = (int) checkInInner.stream()
					.filter(c -> inner[c.y][c.x])
					.count();
		...
		for (int y = 0; y < SIZE; y++) {
			for (int x = 0; x < SIZE; x++) {
				...
				if (grid[y][x]) {
					...
					rating = rating.add(BigInteger.valueOf(2)
							.pow(power));
