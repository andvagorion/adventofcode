...
import net.stefangaertner.util.graph.WeightedGraph;

public class Day18 {

	private static final Predicate<Character> isWall = c -> c == '#';
	private static final Predicate<Character> isEmpty = c -> c == '.';
	private static final Predicate<Character> isItem = isWall.negate()
			.and(isEmpty.negate());
	...
	public static void main(String[] strings) throws IOException {
		...
		List<String> lines = FileUtils.read("aoc19/018-data");
	...
	private static void solve(char[][] grid, List<Point2D> visitedBefore, List<List<Point2D>> solutions) {
		...
		if (graph.getNodes()
			...
			long sum = visitedBefore.stream()
					.mapToLong(p -> (long) p.y)
					.sum();
			if (sum < minimum) {
				String pathStr = visitedBefore.stream()
						.map(p -> ((char) p.x) + " (" + p.y + ")")
						.collect(Collectors.joining(" => "));
				System.out.println(pathStr);
	...
	private static boolean keyCollected(List<Point2D> visited, Integer otherId) {
		int key = (int) String.valueOf((char) otherId.intValue())
				.toLowerCase()
				.charAt(0);
		return visited.stream()
				.map(p -> p.x)
				.anyMatch(i -> i.intValue() == key);
	}
