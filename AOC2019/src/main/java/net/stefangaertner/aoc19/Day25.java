...
public class Day25 {
	...
	private static final List<String> allowedCommands = Arrays
			.asList("north", "south", "east", "west", "take", "drop", "inv");

	private static final List<String> allItemsPath = Arrays.asList("south",
			"south",
			"take hypercube",
			"north",
			"north",
	...
	private static final List<String> items = Arrays.asList("hypercube",
			"coin",
			"klein bottle",
			"shell",
			"easter egg",
	...
	private static void switchToManual(Parser p) throws IOException {
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(System.in))) {
			...
			while (!p.isFinished()) {
				...
				if (allowedCommands.stream()
						.filter(s -> in.startsWith(s))
						.findAny()
						.isPresent()) {
					p.asciiInput(in);
				}
