...
import net.stefangaertner.util.Moon;
...
public class Day12 {
	...
	static long part2() {
		...
		moons.stream()
				.forEach(m -> originals.add(new Moon(m.x, m.y, m.z)));

		int c = 0;
		while (!moons.stream()
				.allMatch(m -> m.rotated)) {
			updateVelocity(moons);
			updatePosition(moons);
		...
		String numbers = moons.stream()
				.map(m -> String.valueOf(m.rot))
