...
public class Battlefield {
	...
	private void move(Unit unit, List<Unit> units) {
		...
		if (!nodes.isEmpty()) {
			...
			// find closest
			Node<Tile> closest = findClosest(nodes);
	...
	private Node<Tile> findClosest(List<Node<Tile>> nodes) {
		...
		for (Node<Tile> node : nodes) {
			...
			if (distance < min) {
				...
				// use reading order
				if (node.data.y * grid[0].length + node.data.x < closest.data.y * grid[0].length + closest.data.x) {
					min = distance;
					closest = node;
				}
			}
	...
	private void printTargets(List<Node<Tile>> nodes, Node<Tile> chosen) {
		...
		for (int y = 0; y < grid.length; y++) {
			printGrid[y] = new String[grid[y].length];
			for (int x = 0; x < grid[y].length; x++) {
				printGrid[y][x] = grid[y][x].toString();
			}
		}
	...
	private boolean nextToEnemy(Tile tile, Unit unit) {
		Unit top = grid[tile.y - 1][tile.x].unit;
		Unit left = grid[tile.y][tile.x - 1].unit;
		Unit right = grid[tile.y][tile.x + 1].unit;
		Unit bottom = grid[tile.y + 1][tile.x].unit;
	...
	public void printUnits() {
		for (Unit u : findUnits()) {
			System.out.println(u.type + " #" + u.id + " (" + u.hp + ")");
		}
	}

	public boolean checkFinished() {
		return getTypesAlive().size() == 1;
	}
	...
	public void printOutcome() {
		...
		if (getTypesAlive().size() > 1) {
			...
			UnitType winner = getTypesAlive().iterator().next();
			int totalHP = getTotalHP();
			System.out.println(winner + " win with " + totalHP + " HP left");
			System.out.println("outcome: " + rounds + " * " + totalHP + " = " + (rounds * totalHP));
	...
	public boolean elvesHaveWon() {
		return getTypesAlive().size() == 1 && getTypesAlive().iterator().next().equals(UnitType.ELF);
	}
}
