...
public class Day17 {
	...
	public static void main(String[] args) throws FileNotFoundException {
		List<String> lines = FileUtils.read("aoc18/017-data");
		...
		int countStill = countChars(area, '~');
		int countFlowing = countChars(area, '|');

		// debugExport(area);

		System.out.println("#1 total: " + countStill + ", " + countFlowing + " => " + (countStill + countFlowing));
		System.out.println("#2 still: " + countStill);
	...
	private static char[][] asArea(List<Rectangle> rects) {
		...
		int offset = rects.stream().map(r -> r.x).min(Comparator.comparing(Integer::valueOf)).get();

		int maxX = rects.stream().map(r -> r.x + r.w).max(Comparator.comparing(Integer::valueOf)).get();
		int maxY = rects.stream().map(r -> r.y + r.h).max(Comparator.comparing(Integer::valueOf)).get();
