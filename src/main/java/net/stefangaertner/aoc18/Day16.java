...
public class Day16 {
	...
	public static void main(String[] args) {
		List<String> lines = FileUtils.read("aoc18/016-data");
	...
	private static void part2(List<String> lines) {
		...
		List<String> program = FileUtils.read("aoc18/016-data2");
		...
		for (int i = 0; i < lines.size(); i += 4) {
			...
			for (OP_CODE op : OP_CODE.values()) {
				...
				if (Arrays.equals(applied, after)) {
					...
					matches.get(op).add(opcode[0]);
				}
			}
		}

		while(!matches.entrySet().isEmpty()) {
			OP_CODE opCode = null;
			Integer opNum = null;
			
			for (Entry<OP_CODE, Set<Integer>> e : matches.entrySet()) {
				// System.out.println(e.getKey() + ": " + e.getValue().stream().map(String::valueOf).collect(Collectors.joining(", ")));
				if (e.getValue().size() == 1) {
					opCode = e.getKey();
					opNum = e.getValue().iterator().next();
					break;
				}
			}
