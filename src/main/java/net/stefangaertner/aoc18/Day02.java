...
public class Day02 {
	...
	public static void main(String[] args) {
		...
		List<String> lines = FileUtils.read("aoc18/002-data");
	...
	private static void part1(List<String> lines) {
		Function<String, Map<Integer, Integer>> freq = line -> line.chars().boxed()
				.collect(Collectors.toMap(c -> c, c -> 1, Integer::sum));
		Function<Map<Integer, Integer>, int[]> ints = m -> new int[] { m.containsValue(2) ? 1 : 0,
				m.containsValue(3) ? 1 : 0 };
		int[] res = lines.stream().map(freq).map(ints).reduce((a, b) -> new int[] { a[0] + b[0], a[1] + b[1] }).get();
		int mult = res[0] * res[1];
		System.out.println("#1 Product is " + mult);
	}
