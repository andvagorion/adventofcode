...
public class Day14 {
	...
	public static void main(String[] strings) throws IOException {
		...
		List<String> lines = FileUtils.read("aoc19/014-example");
		...
		for (String line : lines) {
			...
			Reagent[] reagents = Arrays.stream(inParts).map(s -> new Reagent(s)).toArray(Reagent[]::new);
		...
		Reagent fuel = reactions.entrySet().stream().filter(e -> e.getKey().isFuel()).findFirst().get().getKey();
		fifo.add(fuel);
