...
public class Day07 {
	...
	private static void runPart2(String code) {

//		int out = runLooped(example1_p2, new String[] {"9","8","7","6","5"});
//		System.out.println(out);
//
//		int out2 = runLooped(example2_p2, new String[] {"9","7","8","5","6"});
//		System.out.println(out2);
		...
		for (int i = 0; i < permutations.size(); i++) {
			...
			String[] input = permutations.get(i).split(",");
		...
		System.out.println("Part 2: " + maxAmp);
		System.out.println("        (" + Arrays.stream(combination).collect(Collectors.joining(", ")) + ")");
	}
	...
	private static int runLooped(String code, String[] input) {
		...
		for (int i = 0; i < 5; i++) {
			parsers[i] = Parser.create(code).stopOnOutput().input(input[i]);
		}

		while (!finished) {

			Parser p = parsers[idx];
			p.input(String.valueOf(amplitude)).run();
	...
	private static void runPart1(String code) {
		...
		for (int i = 0; i < permutations.size(); i++) {
			...
			for (int j = 0; j < input.length; j++) {
				...
				Parser p = Parser.create(code).input(in).run();

				int out = Integer.parseInt(p.getOutput().get(0));

//				System.out.println("ACS #" + (j + 1));
...
//				System.out.println("out: " + out);
		...
		System.out.println("Part 1: " + maxAmp);
		System.out.println("        (" + Arrays.stream(combination).collect(Collectors.joining(", ")) + ")");
	}
