...
public class Day20 {
	...
	static long part1() {
		List<List<String>> images = FileUtils.readGroups("aoc20/020");
		Map<Long, Grid2D<Boolean>> grids = images.stream()
				.collect(Collectors.toMap(parseId, parseGrid));
	...
	private static Set<Long> findEdges(Map<Long, Grid2D<Boolean>> grids) {
		Map<Long, List<String>> edgeValues = grids.entrySet()
				.stream()
				.collect(Collectors.toMap(Map.Entry::getKey, bordersToString));

		Set<Long> ids = edgeValues.entrySet()
				.stream()
				.filter(e -> {
					Long id = e.getKey();
					List<String> edges = e.getValue();
					List<List<String>> otherGrids = edgeValues.entrySet()
							.stream()
							.filter(other -> !other.getKey()
									.equals(id))
					...
					return edges.stream()
							.filter(edge -> {
								return otherGrids.stream()
										.anyMatch(other -> {
											return other.stream()
													.anyMatch(str -> {
														return str.equals(edge)
																|| str.equals(StringUtils.reverse(edge));
													});
										});
							})
							.count() == 2;
