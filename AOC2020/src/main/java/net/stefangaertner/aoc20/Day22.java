...
public class Day22 {
	...
	static long part1() {
		List<List<String>> groups = FileUtils.readGroups("aoc20/022");
	...
	private static long playRecursive(Deque<Long> player, Deque<Long> opponent) {
		...
		while (!player.isEmpty() && !opponent.isEmpty()) {
			...
			var config = Pair.of(copyOf(player), copyOf(opponent));
			...
			if (playerCard <= player.size() && opponentCard <= opponent.size()) {
				...
				Deque<Long> playerSub = copyOf(player.stream()
						.limit(playerCard)
						.collect(Collectors.toList()));
				Deque<Long> opponentSub = copyOf(opponent.stream()
						.limit(opponentCard)
						.collect(Collectors.toList()));
