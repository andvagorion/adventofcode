...
public class Day21 {
	...
	static long part1() {
		List<String> lines = FileUtils.read("aoc20/021");

		List<Set<String>> ingredientsList = new ArrayList<>();
		Map<String, Set<Set<String>>> a2i = new HashMap<>();
		Map<String, Set<String>> i2a = new HashMap<>();
		...
		lines.forEach(line -> {
			...
			Set<String> ingredients = Arrays.stream(in1.split(" "))
					.collect(Collectors.toSet());
			ingredientsList.add(ingredients);

			Set<String> allergens = Arrays.stream(in2.split(", "))
					.collect(Collectors.toSet());
	...
	private static Map<String, String> evaluate(Map<String, Set<String>> i2a, Map<String, Set<Set<String>>> a2i) {
		...
		Map<String, Set<String>> remaining = i2a.entrySet()
				.stream()
				.filter(e -> !e.getValue()
						.isEmpty())
		...
		while (!remaining.isEmpty()) {
			...
			Set<String> singleAllergens = a2i.keySet()
					.stream()
					.filter(a -> {
	...
	private static void remove(Map<String, Set<String>> i2a, Map<String, Set<Set<String>>> a2i) {
		...
		Set<String> ingredients = i2a.keySet()
				.stream()
				.collect(Collectors.toSet());

		ingredients.forEach(ingredient -> {
			Set<String> possibleAllergens = i2a.get(ingredient)
					.stream()
					.collect(Collectors.toSet());

			// check if it's in all lists
			for (String allergen : possibleAllergens) {
				boolean possible = a2i.get(allergen)
						.stream()
						.allMatch(set -> set.contains(ingredient));
				if (!possible) {
					i2a.get(ingredient)
							.remove(allergen);
				}
