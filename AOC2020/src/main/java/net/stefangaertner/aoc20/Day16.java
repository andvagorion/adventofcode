...
public class Day16 {
	...
	static long part2() {
		...
		List<Set<IntPair>> validRanges = parseValidRanges(lines);
		Set<List<Integer>> nearby = parseTickets(lines, "nearby tickets");
		List<Integer> ownTicket = parseTickets(lines, "your ticket").iterator()
				.next();

		Set<List<Integer>> validTickets = nearby.stream()
				.filter(list -> {
					return list.stream()
							.allMatch(i -> {
								return validRanges.stream()
		...
		for (List<Integer> ticket : validTickets) {
			possibleCombinations = possibleCombinations.stream()
					.filter(possibleCombination -> {
						...
						return ranges.stream()
								.anyMatch(p -> ticketValue >= p.a && ticketValue <= p.b);
					})
					.collect(Collectors.toSet());
		}
		...
		while (!possibilities.isEmpty()) {
			...
			List<IntPair> match = possibilities.values()
					.stream()
			...
			possibilities.forEach((k, v) -> {
				changed.put(k,
						v.stream()
								.filter(pair -> pair.b != single.b)
								.collect(Collectors.toList()));
			});

			possibilities = changed.entrySet()
					.stream()
					.filter(e -> !e.getValue()
							.isEmpty())
		...
		return finalMapping.stream()
				.filter(pair -> {
					return fields.get(pair.a)
	...
	private static Set<List<Integer>> parseTickets(List<String> lines, String header) {
		...
		while (!lines.get(lineIndex)
				.isEmpty()) {
			out.add(Arrays.stream(lines.get(lineIndex++)
					.split(","))
