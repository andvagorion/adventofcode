...
import net.stefangaertner.util.PointHex;
import net.stefangaertner.util.PointHex.HexDirection;
...
public class Day24 {
	...
	static long part1() {
		List<String> lines = FileUtils.read("aoc20/024");
	...
	private static void printWhole(Set<PointHex> flipped) {
		int minY = flipped.stream()
				.min(Comparator.comparing(p -> p.y))
				.get().y;
		int maxY = flipped.stream()
				.max(Comparator.comparing(p -> p.y))
				.get().y;

		int minX = flipped.stream()
				.min(Comparator.comparing(p -> p.x))
				.get().x;
		int maxX = flipped.stream()
	...
	private static void printTrail(List<PointHex> trail) {
		int minY = trail.stream()
				.min(Comparator.comparing(p -> p.y))
				.get().y;
		int maxY = trail.stream()
				.max(Comparator.comparing(p -> p.y))
				.get().y;

		int minX = trail.stream()
				.min(Comparator.comparing(p -> p.x))
				.get().x;
		int maxX = trail.stream()
				.max(Comparator.comparing(p -> p.x))
				.get().x;
