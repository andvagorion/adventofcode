...
import net.stefangaertner.util.Filter;
...
public class Day19 {
	...
	static long part1() {
		List<List<String>> groups = FileUtils.readGroups("aoc20/019");
		Map<Integer, String> ruleset = getRuleset(groups.get(0));
		return Filter.list(groups.get(1))
				.matchRegex(ruleset.get(0))
				.get()
				.size();
	}
	...
	static long part2() {
		...
		List<String> filtered = Filter.list(initialRules)
				.remove(str -> str.startsWith("8:") || str.startsWith("11: "))
				.get();
		...
		List<String> matching = Filter.list(groups.get(1))
				.matchRegex(ruleset.get(0))
				.get();
	...
	private static Map<Integer, String> getRuleset(List<String> rules) {
		...
		List<String> compoundRules = Filter.list(rules)
				.doesNotMatchRegex(basicRulePattern)
				.get();
	...
	private static Map<Integer, String> evaluateCompoundRules(Map<Integer, String> ruleset,
		...
		while (!compoundRules.isEmpty()) {
			...
			for (String compoundRule : compoundRules) {
				...
				if (!nums.stream()
						.allMatch(num -> ruleset.containsKey(num))) {
	...
	private static Map<Integer, String> evaluateBasicRules(List<String> rules) {
		return rules.stream()
				.filter(line -> line.matches(basicRulePattern))
	...
	private static Map<Integer, String> addAdditionalRules(Map<Integer, String> ruleset) {
		...
		int highestRuleNumber = ruleset.keySet()
				.stream()
