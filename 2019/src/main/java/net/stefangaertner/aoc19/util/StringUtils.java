...
public class StringUtils {
	...
	public static <T extends Enum<?>> LinkedList<T> parseSymbols(String line, List<T> symbols) {
		...
		while (!characters.isEmpty()) {
			...
			while (!symbols.stream().anyMatch(e -> e.name().equals(symbol.toString().toUpperCase()))) {
				symbol.append(characters.pop());
			}

			T obj = symbols.stream().filter(e -> e.name().equals(symbol.toString().toUpperCase())).findFirst()
					.orElse(null);
	...
	public static LinkedList<Character> asList(final String string) {
		return string.chars().mapToObj(e -> (char) e).collect(Collectors.toCollection(() -> new LinkedList<>()));
	}
